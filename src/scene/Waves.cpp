...
std::pair<glm::vec3, glm::vec3> PointWave::computeBinormalAndTangent(float absoluteTime, glm::vec3 position) const {
    ...
    float phaseXderivate = (position.x - origin.x) / (std::sqrt(waveLength) * -length(originToVert));
    float phaseZderivate = (position.z - origin.y) / (std::sqrt(waveLength) * -length(originToVert));
